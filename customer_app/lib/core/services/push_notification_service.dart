...
class PushNotificationService {
  ...
  static Future<void> registerDeviceToken() async {
    ...
    try {
      ...
      if (token == null || token.isEmpty) {
        if (kDebugMode) {
          debugPrint('[CustomerPush] FCM token is empty');
        }
        return;
      }
      ...
      if (kDebugMode) {
        debugPrint('[CustomerPush] registerDeviceToken failed: $e');
      }
      // Push registration must never block login or checkout.
    }
  }

  static Future<String?> _resolveFcmTokenWithRetry() async {
    for (var i = 0; i < 5; i++) {
  ...
  static Future<void> _sendToken(String token) async {
    ...
    for (var i = 0; i < 3; i++) {
      response = await ApiClient.post('/fcm-token', {
        'token': token,
        'platform': _platform,
      });
      if (response['success'] == true || response['status'] == true) {
        if (kDebugMode) {
          debugPrint('[CustomerPush] token registered successfully');
        }
        return;
      }
